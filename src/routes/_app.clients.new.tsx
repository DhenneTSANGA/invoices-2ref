...
function NewClient() {
  ...
  return (
    <div>
      ...
      <PageHeader title="Nouveau client" subtitle="Renseignez les informations légales et de contact." />
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-gradient-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-glow"><Save className="h-4 w-4" /> Enregistrer</button>
        </div>
      </form>
    </div>
  );
...
function Field({ label, value, onChange, type = "text", placeholder, required, colSpan }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string; required?: boolean; colSpan?: number }) {
  return (
    <label className={colSpan === 2 ? "sm:col-span-2 block" : "block"}>
      <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{label}{required && <span className="text-danger"> *</span>}</span>
      <input type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} required={required} className="mt-1 w-full rounded-xl border border-border/60 bg-transparent px-3 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition" />
    </label>
  );
}
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: string[] }) {
  return (
    <label className="block">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full rounded-xl border border-border/60 bg-surface px-3 py-2.5 text-sm focus:border-primary focus:outline-none">
