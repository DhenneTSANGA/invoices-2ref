...
import {
  Eye, Plus, Search, Send, Ban, Filter, FileText, CheckCircle2, XCircle,
} from "lucide-react";
...
const QUOTATION_STATUSES: DocumentStatus[] = ["draft", "sent", "accepted", "rejected", "cancelled"];
...
function QuotationsPage() {
  ...
  const docs = useMemo(
    () => documents.filter((d) => d.type === "quotation"),
    [documents],
  );

  const counts = useMemo(() => {
    const c: Record<string, number> = { all: docs.length };
    for (const s of QUOTATION_STATUSES) c[s] = docs.filter((d) => d.status === s).length;
    return c;
  }, [docs]);
  ...
  const changeStatus = (id: string, next: DocumentStatus, number: string) => {
    ...
    toast.success(`Devis ${number}`, { description: statusLabel(next) });
  };
  ...
  return (
    <div>
      <PageHeader
        title="Devis"
        subtitle="Propositions commerciales — envoyés, acceptés, refusés ou annulés"
        actions={
          <Link to="/quotations/new" className="inline-flex items-center gap-2 rounded-2xl bg-gradient-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-glow">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        <StatusCard label="Tous" count={counts.all} active={status === "all"} onClick={() => setStatus("all")} accent="bg-slate-500" />
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <Kpi
          label="Acceptés"
          value={currency(docs.filter((d) => d.status === "accepted").reduce((a, b) => a + b.total, 0))}
        />
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          action={
            <Link to="/quotations/new" className="rounded-xl bg-gradient-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-glow">
              Créer un devis
            </Link>
          }
        />
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead className="bg-muted/60 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-5 py-3 text-left">Validité</th>
                ...
                return (
                  <motion.tr
                    ...
                    className={cn(
                      ...
                      d.status === "accepted" && "bg-emerald-50/40",
                      d.status === "rejected" && "bg-orange-50/30",
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {d.status !== "sent" && d.status !== "accepted" && d.status !== "cancelled" && (
                        ...
                        {d.status !== "accepted" && d.status !== "cancelled" && (
                          <IconBtn title="Accepter" className="text-emerald-600 hover:bg-emerald-50" onClick={() => changeStatus(d.id, "accepted", d.number)}>
                            <CheckCircle2 className="h-4 w-4" />
                          </IconBtn>
                        )}
                        {d.status !== "rejected" && d.status !== "cancelled" && (
                          <IconBtn title="Refuser" className="text-orange-600 hover:bg-orange-50" onClick={() => changeStatus(d.id, "rejected", d.number)}>
