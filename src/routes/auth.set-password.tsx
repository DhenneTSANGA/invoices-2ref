...
export const Route = createFileRoute("/auth/set-password")({
  head: () => ({ meta: [{ title: "Définir le mot de passe — 2R Hub" }] }),
  beforeLoad: async () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userErr || !userData.user) {
        throw new Error("Session expirée — rouvrez le lien d’invitation");
      }
      ...
      toast.success("Mot de passe enregistré", {
        description: "Vous pouvez désormais vous connecter avec e-mail et mot de passe.",
      });
      ...
      toast.error(
        humanAuthError(err, "Impossible d’enregistrer le mot de passe pour le moment."),
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="aurora-bg flex min-h-screen items-center justify-center p-6">
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Votre invitation est validée. Choisissez un mot de passe pour vos
          prochaines connexions à 2R Hub.
        </p>
